(() => {
  class P10QA extends Gatha { // Manjusri asks 9 budhisattvas; all ask him.
    constructor(id, pusa, question, gatha) {
      super(id, gatha, question); this.pusa = pusa;
      var pref;
      if (pusa == '文殊師利') // others ask Manjusri
        pref = '爾時，諸菩薩謂文殊師利菩薩言：';
      else // Manjusri ask another budhisattva
        pref = `爾時，文殊師利菩薩問${c(pusa)}菩薩言：`;
      this.lead = `${pref}：<a name="${this.id}" this.id="${this.id}"></a>「${this.info}」

時，${c(this.pusa)}菩薩以頌答曰：
...
`;
  ...
  for (var i in a) {
    var arr = a[i],
        ln = arr.shift(), q = arr.shift(),
        b = ln.substring(1).split(' '), id = b[0], pusa = b[1];
    new P10QA(id, pusa, q, arr.join('\n'));
  }
})();
