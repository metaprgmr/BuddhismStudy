function showWLSJs(curname) {
  ...
  function book(name, vowNum, zis, ttl, before, after) {
    before = (before||'夏集本✶').replace('✶', `&nbsp;&nbsp;<font style="font-size:14px">(${vowNum}願。${zis}字)</font>`);
    if (after) after += '<br>';
    if (name == 'Xia')
...
　book('HAN', 24, 28224, '無量清淨平等覺經', '　【漢譯】✶',     z11 + '　後漢月氏沙門支婁迦讖譯');
  book('WU',  24, 25124, '佛說阿彌陀三耶三佛薩樓佛檀過度人道經', '　【吳譯】✶', '　吳月氏優婆塞支謙字恭明譯');
  book('WEI', 48, 17381, '無量壽經', '　【魏譯】✶',             z11 + '　　　　　　曹魏印度沙門康僧鎧譯');
  book('TANG',48, 14003, '無量壽如來會', '　【唐譯】✶',         z11 + '　　　唐南印度三藏菩提流志譯');
  book('SONG',36, 11661, '佛說大乘無量壽莊嚴經', '　【宋譯】✶', z11 + '　　宋西域沙門法賢譯');
  book('Wang',48, 17739, '大阿彌陀經', '【王校本】✶',           z11 + '　　　　　　　　王日休居士校輯');
  book('Xia', 48, 16171);
